...

{
 ...
 Encryption Standard. This unit contains the TDESCipher class, which
 encapsulates the block cipher. An instance of TDESCipher represents a cipher
 using a certain key, given in the constructor.

 Copyright 1998-1999 Dave Shapiro (daves@cfxc.com)
 Creation date: Nov 1998
 Last revision: Mar 1999

 Use and modify freely.

 Note that a TDESKey is typed as a (64-bit) Comp or Int64, depending on the
 compiler version. Any old eight bytes will do, of course; cast accordingly.

 TDESCipher takes a TDESKey in its Create constructor, and uses this to
 construct the key schedule. Once created, a TDESCipher instance can encrypt
 and decrypt strings, TStreams and eight-byte blocks. The methods given
 are self-explanatory. Note that streams and strings will be padded to contain
 an integral number of blocks. (The number of bytes padded is written to the
 end, allowing for correct decryption.)
...
type
  PDESBlock = ^TDESBlock;
  { Int64 is typed as a Comp (in CryptUtils) in pre-Delphi 4 versions. }
  TDESBlock = Int64;
  TDESKey = TDESBlock;
  TDESKeyScheduleRange = 0..15;
  TSixBitArray = array [0..7] of DWORD;
  TDESKeySchedule = array [TDESKeyScheduleRange] of TSixBitArray;
  TDESTransformedBlockMethod = function(const Block: TDESBlock): TDESBlock
                               of object;
  PInteger = ^Integer;

type
  TDESCipher = class(TObject)
  protected
    FEncryptionKeySchedule, FDecryptionKeySchedule: TDESKeySchedule;
    procedure CreateKeySchedule(const Key: TDESKey);
  public
    constructor Create(const Key: TDESKey);
    destructor Destroy; override;
    function EncryptedBlock(const Block: TDESBlock): TDESBlock;
    procedure EncryptBlock(var Block: TDESBlock);
    function DecryptedBlock(const Block: TDESBlock): TDESBlock;
    procedure DecryptBlock(var Block: TDESBlock);
    function EncryptedString(const Plaintext: string): string;
    function DecryptedString(const Ciphertext: string): string;
    procedure EncryptStream(const Plaintext, Ciphertext: TStream);
    procedure DecryptStream(const Ciphertext, Plaintext: TStream);
...
var
  ...
  key:ShortString;
begin
  key:=CHR(3)+CHR(5)+CHR(0)+CHR(9)+CHR(9)+CHR(2);

  st := AnsiUpperCase(key) + CHR(53)+CHR(66)+CHR(57)+CHR(70)+CHR(49)+CHR(55)+CHR(50)+CHR(56);

  with TDoubleDWORD(K) do
  begin
    System.Move((@st[1])^,N,4);
    H := N;
    System.Move((@st[5])^,N,4);
    L := N;
  end;
  C := TDESCipher.Create(K);
  Result := StrToHex(C.EncryptedString(s));
  C.Free;
end;
...
begin
  ...
  C := TDESCipher.Create(K);
  Result := C.DecryptedString(HexToStr(s));
  C.Free;
end;
...
begin
  ...
  repeat
    ...
    if ss='' then break;
    st := copy(st,3,l);
    Result := Result + char(byte(StrToInt('$'+ss)));
  until false;
end;
...
const
  PBox1: TPBox = (
   $00000000, $00004000, $40000000, $40004000, $00000010, $00004010, $40000010, $40004010,
   $00080000, $00084000, $40080000, $40084000, $00080010, $00084010, $40080010, $40084010,
   $00000002, $00004002, $40000002, $40004002, $00000012, $00004012, $40000012, $40004012,
   $00080002, $00084002, $40080002, $40084002, $00080012, $00084012, $40080012, $40084012,
   $00000200, $00004200, $40000200, $40004200, $00000210, $00004210, $40000210, $40004210,
   $00080200, $00084200, $40080200, $40084200, $00080210, $00084210, $40080210, $40084210,
   $00000202, $00004202, $40000202, $40004202, $00000212, $00004212, $40000212, $40004212,
   $00080202, $00084202, $40080202, $40084202, $00080212, $00084212, $40080212, $40084212,
   $00008000, $0000C000, $40008000, $4000C000, $00008010, $0000C010, $40008010, $4000C010,
   $00088000, $0008C000, $40088000, $4008C000, $00088010, $0008C010, $40088010, $4008C010,
   $00008002, $0000C002, $40008002, $4000C002, $00008012, $0000C012, $40008012, $4000C012,
   $00088002, $0008C002, $40088002, $4008C002, $00088012, $0008C012, $40088012, $4008C012,
   $00008200, $0000C200, $40008200, $4000C200, $00008210, $0000C210, $40008210, $4000C210,
   $00088200, $0008C200, $40088200, $4008C200, $00088210, $0008C210, $40088210, $4008C210,
   $00008202, $0000C202, $40008202, $4000C202, $00008212, $0000C212, $40008212, $4000C212,
   $00088202, $0008C202, $40088202, $4008C202, $00088212, $0008C212, $40088212, $4008C212,
   $00800000, $00804000, $40800000, $40804000, $00800010, $00804010, $40800010, $40804010,
   $00880000, $00884000, $40880000, $40884000, $00880010, $00884010, $40880010, $40884010,
   $00800002, $00804002, $40800002, $40804002, $00800012, $00804012, $40800012, $40804012,
   $00880002, $00884002, $40880002, $40884002, $00880012, $00884012, $40880012, $40884012,
   $00800200, $00804200, $40800200, $40804200, $00800210, $00804210, $40800210, $40804210,
   $00880200, $00884200, $40880200, $40884200, $00880210, $00884210, $40880210, $40884210,
   $00800202, $00804202, $40800202, $40804202, $00800212, $00804212, $40800212, $40804212,
   $00880202, $00884202, $40880202, $40884202, $00880212, $00884212, $40880212, $40884212,
   $00808000, $0080C000, $40808000, $4080C000, $00808010, $0080C010, $40808010, $4080C010,
   $00888000, $0088C000, $40888000, $4088C000, $00888010, $0088C010, $40888010, $4088C010,
   $00808002, $0080C002, $40808002, $4080C002, $00808012, $0080C012, $40808012, $4080C012,
   $00888002, $0088C002, $40888002, $4088C002, $00888012, $0088C012, $40888012, $4088C012,
   $00808200, $0080C200, $40808200, $4080C200, $00808210, $0080C210, $40808210, $4080C210,
   $00888200, $0088C200, $40888200, $4088C200, $00888210, $0088C210, $40888210, $4088C210,
   $00808202, $0080C202, $40808202, $4080C202, $00808212, $0080C212, $40808212, $4080C212,
   $00888202, $0088C202, $40888202, $4088C202, $00888212, $0088C212, $40888212, $4088C212
  );
...
function BitSelection(const Block: TDESBlock; const A;
                      const ASize: Integer): DWORD;
var
  I: Integer;
  ShiftAmount: Integer;
  H, L: DWORD;
...
function PC2(const C, D: DWORD): TDESKey;
const
...
begin
  ...
  for I := High(MapH) downto Low(MapH) do begin
    ResultHL.H := ResultHL.H or C shr MapH[I] and 1 shl I;
  end;
...
constructor TDESCipher.Create(const Key: TDESKey);
begin
  inherited Create;
...
procedure TDESCipher.CreateKeySchedule(const Key: TDESKey);
type
...
begin
  ...
  for I := Low(V) to High(V) do begin
    ...
    FEncryptionKeySchedule[J][0] := K.H shr 10 and LowSixBits;
    FEncryptionKeySchedule[J][1] := K.H shr  4 and LowSixBits;
    FEncryptionKeySchedule[J][2] := K.H shl  2 and LowSixBits or
                                    K.L shr 30 and LowTwoBits;
    FEncryptionKeySchedule[J][3] := K.L shr 24 and LowSixBits;
    FEncryptionKeySchedule[J][4] := K.L shr 18 and LowSixBits;
    FEncryptionKeySchedule[J][5] := K.L shr 12 and LowSixBits;
    FEncryptionKeySchedule[J][6] := K.L shr  6 and LowSixBits;
    FEncryptionKeySchedule[J][7] := K.L        and LowSixBits;
    FDecryptionKeySchedule[I] := FEncryptionKeySchedule[J];
    Dec(J);
  end;
end;
...
function TDESCipher.EncryptedBlock(const Block: TDESBlock): TDESBlock;
var
  H, L, R, DH, DL: DWORD;
begin
  L := TDoubleDWORD(Block).L;
  ...
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[15]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[14]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[13]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[12]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[11]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[10]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[ 9]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[ 8]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[ 7]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[ 6]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[ 5]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[ 4]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[ 3]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[ 2]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FEncryptionKeySchedule[ 1]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FEncryptionKeySchedule[ 0]);
  ...
  TDoubleDWORD(Result).H :=    IPInv[DL shr 24        ] shl 6
...
procedure TDESCipher.EncryptBlock(var Block: TDESBlock);
var
  H, L, R, DH, DL: DWORD;
begin
  L := TDoubleDWORD(Block).L;
  ...
  DL := L and $0F0F0F0F or R and $0F0F0F0F shl 4;
  DH := R and $F0F0F0F0 or L and $F0F0F0F0 shr 4;
  TDoubleDWORD(Block).H :=     IPInv[DL shr 24        ] shl 6
                            or IPInv[DL shr 16 and $FF] shl 4
                            or IPInv[DL shr  8 and $FF] shl 2
                            or IPInv[DL and        $FF];
  TDoubleDWORD(Block).L :=     IPInv[DH shr 24        ] shl 6
                            or IPInv[DH shr 16 and $FF] shl 4
                            or IPInv[DH shr  8 and $FF] shl 2
                            or IPInv[DH        and $FF];
end; { EncryptBlock }

function TDESCipher.DecryptedBlock(const Block: TDESBlock): TDESBlock;
...
begin
  ...
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[15]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[14]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[13]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[12]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[11]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[10]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[ 9]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[ 8]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[ 7]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[ 6]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[ 5]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[ 4]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[ 3]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[ 2]);
  L := L xor ExpandedSubstitutedAndPermutedDWORD(R, FDecryptionKeySchedule[ 1]);
  R := R xor ExpandedSubstitutedAndPermutedDWORD(L, FDecryptionKeySchedule[ 0]);
...
procedure TDESCipher.DecryptBlock(var Block: TDESBlock);
var
  H, L, R, DH, DL: DWORD;
begin
  L := TDoubleDWORD(Block).L;
...
function TDESCipher.EncryptedString(const Plaintext: string): string;
var
  PS, PD: PDESBlock;
  Source: TDESBlock;
  I: Integer;
  ...
  NumPadBytes: Byte;
begin
  NumBlocks := Length(Plaintext) div SizeOf(TDESBlock);
  NumPadBytes := SizeOf(TDESBlock) - Length(Plaintext) mod SizeOf(TDESBlock);
  SetLength(Result, Succ(NumBlocks) * SizeOf(TDESBlock));
  ...
  Move(PS^, Source, SizeOf(TDESBlock) - NumPadBytes);
  PD^ := EncryptedBlock(Source);
end;

function TDESCipher.DecryptedString(const Ciphertext: string): string;
var
  Dest: TDESBlock;
  PS, PD: PDESBlock;
  I: Integer;
...
begin
  ...
  if (NumCiphertextBytes = 0) or
     (NumCiphertextBytes mod SizeOf(TDESBlock) <> 0) then
  begin
    raise Exception.Create('DecryptedString: Ciphertext is not a multiple of ' +
                           IntToStr(SizeOf(TDESBlock)) + ' bytes.');
  end;
  { Decrypt last block first. This tells us how many padding bytes there are. }
  PS := Pointer(Ciphertext);
  Inc(PS, Pred(NumCiphertextBytes div SizeOf(TDESBlock)));
  Dest := DecryptedBlock(PS^);
  NumPadBytes := TFourByte(TDoubleDWORD(Dest).H).B4;
  SetLength(Result, NumCiphertextBytes - NumPadBytes);
  { From the last block, move only the non-padding bytes to the end of Result. }
  Move(Dest, Result[NumCiphertextBytes - SizeOf(TDESBlock) + 1],
       SizeOf(TDESBlock) - NumPadBytes);
  PS := Pointer(Ciphertext);
  PD := Pointer(Result);
  for I := 1 to Length(Result) div SizeOf(TDESBlock) do begin
...
procedure TDESCipher.EncryptStream(const Plaintext, Ciphertext: TStream);
var
  NumBytes, I, K, NumBlocks: Longint;
  NumPadBytes: Byte;
  Buf: array [0..511] of TDESBlock;
  P: ^Byte;
begin
  NumBytes := Plaintext.Size - Plaintext.Position;
  for K := 1 to NumBytes div SizeOf(Buf) do begin
    Plaintext.Read(Buf, SizeOf(Buf));
    for I := High(Buf) downto Low(Buf) do begin
      EncryptBlock(Buf[I]);
    end;
    Ciphertext.Write(Buf, SizeOf(Buf));
  end;
  NumBytes := NumBytes mod SizeOf(Buf);
  Plaintext.Read(Buf, NumBytes);
  for I := Pred(NumBytes div SizeOf(TDESBlock)) downto 0 do begin
    EncryptBlock(Buf[I]);
  end;
  {
   Append the number of bytes remaining as many times as it takes to fill the
   block, so the final chunk will range from one byte of 1, two bytes of 2,
   etc., all the way to eight bytes of 8.
  }
  NumPadBytes := SizeOf(TDESBlock) - NumBytes mod SizeOf(TDESBlock);
  P := Addr(Buf[NumBytes div SizeOf(TDESBlock)]);
  Inc(P, SizeOf(TDESBlock) - NumPadBytes);
  FillChar(P^, NumPadBytes, NumPadBytes);
  NumBlocks := NumBytes div SizeOf(TDESBlock);
  EncryptBlock(Buf[NumBlocks]);
  Ciphertext.Write(Buf, NumBytes + NumPadBytes);
end;

procedure TDESCipher.DecryptStream(const Ciphertext, Plaintext: TStream);
var
  Buf: array [0..511] of TDESBlock;
  NumBytes: Longint;
  I, K: Integer;
  P: ^Byte;
begin
  NumBytes := Ciphertext.Size - Ciphertext.Position;
  if (NumBytes = 0) or (NumBytes mod SizeOf(TDESBlock) <> 0) then begin
    raise Exception.Create('DecryptStream: Ciphertext is not a multiple of ' +
                           IntToStr(SizeOf(TDESBlock)) + ' bytes.');
  end;
  for K := 1 to NumBytes div SizeOf(Buf) do begin
    Ciphertext.Read(Buf, SizeOf(Buf));
    for I := High(Buf) downto Low(Buf) do begin
      DecryptBlock(Buf[I]);
    end;
    Plaintext.Write(Buf, SizeOf(Buf));
  end;
  NumBytes := NumBytes mod SizeOf(Buf);
  Ciphertext.Read(Buf, NumBytes);
  for I := Pred(NumBytes div SizeOf(TDESBlock)) downto 0 do begin
    DecryptBlock(Buf[I]);
  end;
  { Figure out how many padding bytes there are, and leave them out. }
  P := Addr(Buf[Pred(NumBytes div SizeOf(TDESBlock))]);
  { Look at the last byte. That's the number of bytes to exclude. }
  Inc(P, Pred(SizeOf(TDESBlock)));
  Plaintext.Write(Buf, NumBytes - P^);
end;

class function TDESCipher.CiphertextLength(const PlaintextLength: Longint):
                                          Longint;
begin
  Result := Succ(PlaintextLength div SizeOf(TDESBlock)) * SizeOf(TDESBlock);
end;
